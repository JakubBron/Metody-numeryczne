function [integration_error, Nt, ft_5, integral_1000] = zadanie1()
    ...
    integration_error = [];
    for i = 1:length(Nt)
		integration_error(i) = abs(trapeze_integration(@f, Nt(i)) - reference_value);
	end

    ft_5 = f(5);
    integral_1000 = trapeze_integration(@f, 1000);
...
function result = trapeze_integration(f, N)
    a = 0;
    b = 5;
    h = (b-a)/N;
    result = 0;
