    ...
    with open(securityName+'_'+period+'_profitloss.txt', 'w') as f:
        ...
        for i in range(0, len(BUYSELL)):
            if BUYSELL[i][1] == "BUY" and sell == [0,0]:
                if initialCapital == 0:
                    initialCapital = price[i] * 1000
                
                buy = [price[i], BUYSELL[i][0], 1000]
                cumulativeBuy = cumulativeBuy + buy[0]*buy[2]
            
            if BUYSELL[i][1] == "SELL" and buy != [0,0,0]:
                sell = [price[i], BUYSELL[i][0]]
                cumulativeSell = cumulativeSell + sell[0]*buy[2]
                profit = (float(sell[0]) - float(buy[0])) * buy[2]
                
                profitMoney = "%.2f"%round(profit, 2)
                profitPercentage = "%.3f"%round(profit/(buy[0]*buy[2]) * 100, 3)

                report = "Buy date: "+buy[1].strftime('%Y-%m-%d')+ \
                        " | Buy price: "+f"{ '%.2f'%round(buy[0], 2):<10}" + \
                        " | Sell date: "+sell[1].strftime('%Y-%m-%d') + \
                        " | Sell price: "+f"{'%.2f'%round(sell[0], 2):<10}" + \
                        " | Profit: "+ f"{profitMoney:<10}" +" "+ f"{profitPercentage:<7}" + "%" + \
                        " | Duration: "+ str(sell[1]-buy[1])
        ...
        print("=========================================================================================================================================================", file=f)
        cumulativeProfitLoss = cumulativeSell-cumulativeBuy
        print("Total earnings: " + str(round(cumulativeProfitLoss, 2)) + " generated using " + str(round(initialCapital)) + " units of borrowed capital, which needs to be returned", file=f)
        cumulativeProfitLoss = cumulativeProfitLoss - initialCapital
        profitLossPercentage = cumulativeProfitLoss/initialCapital * 100
        print("Return on Investment ROI: "+ str(round(profitLossPercentage, 3)) + "%", file=f)   
        print("\n", file=f)
    f.close()
...
worker(SP500, 'SP500', 'D1', True)
worker(PKN, 'PKN', 'D1', True)
